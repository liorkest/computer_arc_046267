	...
	public:
		...
		uint32_t cache_read(uint32_t data_address)
		{
			...
			if(way_idx_of_data == -1) // not found in the cache
			{
				...
				add_block_to_cache(data_address);
			}
			else 
			{ 
		...
		uint32_t cache_write(uint32_t data_address)
		{
			...
			else
			{
				...
				if(allocate){
					...
					if (lower_cache!=NULL){
						way_idx_of_data = search_in_cache(data_address);
						ways_list[way_idx_of_data].set_dirty_bit(data_address, true);
					}
					return time;
				}	
		...
		int add_block_to_cache(uint32_t data_address) // returns the index of way that the data was added to 
		{
			...
			if(!got_added)
			{
				...
				for(unsigned i=0 ; i< num_of_ways ; i++){
					if(ways_list[i].get_age_of_block(data_address) == 1)//found LRU
					{
						...
						{
							// update the LRU of the block in lower cache using add_block_to_cache
							lower_cache->add_block_to_cache(ways_list[i].get_address_of_existing_block(data_address)); 
						}
			...
			update_ages(data_address, updated_way);  
		...
		void print(){
			...
			printf("cache_size=%d, block_size=%d, allocate=%d, mem_cycles=%d, cache_cycles=%d, lower_cache(pointer)=%d, num_of_ways=%d\n",cache_size,block_size_bytes, allocate,mem_cycles, cache_cycles,lower_cache,num_of_ways);
